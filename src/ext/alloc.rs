...
pub trait ToStringExt: ToString {
    fn dis_to_error(&self) -> Displayish;
    fn dis_to_error_with<FD: Display, F: Fn() -> FD>(&self, f: F) -> Displayish;
...
pub trait ResultErrToDisplayExt<T> {
    fn dis_map_err_to(self) -> DisplayishResult<T>;
    fn dis_map_err_to_with<FD: Display, F: Fn() -> FD>(self, f: F) -> DisplayishResult<T>;
...
pub trait ResultDisplayToString<T, EX> {
    fn dis_to_string_based(self) -> DisplayishResult<T, String, EX>;
