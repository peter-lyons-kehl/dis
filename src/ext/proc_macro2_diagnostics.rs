...
pub trait AsMacroDeepDiagnostic<D: Display = String> {
    fn dis_as_macro_deep_diagnostic(self) -> MacroDeepDiagnostic<D>;
...
pub trait DebugExt: Debug {
    fn dis_dbg_error_at(&self, span: Span) -> PmDiagnostic;

    fn dis_dbg_error_with_at<FD: Display, F: Fn() -> FD>(self, f: F, span: Span) -> PmDiagnostic;
...
pub trait AsDeepMacroResult<T, D: Display> {
    #[allow(private_interfaces)]
    fn dis_map_macro_err(self) -> MacroDeepResult<T, D>;
}
...
pub trait ToStringExt: ToString {
    fn dis_to_error_at(&self, span: Span) -> PmDiagnostic;
    fn dis_to_error_with_at<FD: Display, F: Fn() -> FD>(&self, f: F, span: Span) -> PmDiagnostic;
...
pub trait ResultErrToDisplayExt<T> {
    fn dis_map_err_to_at(self, span: Span) -> MacroDiagnosticResult<T>;
    fn dis_map_err_to_with_at<FD: Display, F: Fn() -> FD>(
        self,
        f: F,
        span: Span,
...
pub trait MacroDeepResultExt<T> {
    ...
    fn dis_span_err(self, span: Span) -> MacroDiagnosticResult<T>;
